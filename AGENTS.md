...
Codex is the Software Engineer, Implementer, and Tester for the apartment-manager Planner app.
...
<!-- ══════════════════════════════════════════════════════════════════════════
     FILL IN the two blocks below. They are the ONLY app-specific parts of this
     file. Everything else is the OS and should be left alone.

     These rules MUST match CLAUDE.md's Hard Rules 3-7. If they drift, Codex and
     Claude are enforcing different contracts and the review gate becomes noise.
     ══════════════════════════════════════════════════════════════════════════ -->

TODO: match the existing style. State it concretely — the architecture, and what you must not
introduce (framework? build step? module system?). Cite the decision that locked it in.
...
- **TODO: an app hard rule.** State the mistake, then the production consequence, then the
  decision that records it. Write these only from real bugs — an invented rule teaches Codex the
  whole list is advisory.
- **TODO**
- **When referencing code locations in `CHANGELOG.md` or `TEST_REPORT.md`, use stable anchors**
  (function/object names, DOM ids, DB paths, storage keys) — never line numbers. They rot on the
  next edit. See `docs/DECISIONS.md` D-008.

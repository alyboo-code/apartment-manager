...
```powershell
cd "/Users/alyssamarieborbon/Downloads/Vibe coding/apartment-manager"
.\run-claude.ps1
```
...
| Task | Fires | Job |
|---|---|---|
| **apartment-manager Command Dispatcher** | every **30 min** | Drains Telegram commands (`/status`, `/go`, `/build`, `/review`, `/merge`…) |
| **apartment-manager Claude Overnight** | **9 PM & 2 AM** | Autonomous run: triage captures → proposals → morning digest |
...
| Name | Type | Value |
|---|---|---|
| `Telegram Bot - apartment-manager` | Telegram | that app's bot token |
...
```powershell
Get-ScheduledTask -TaskName "apartment-manager Command Dispatcher" |
  Select-Object State, @{n='Wake';e={$_.Settings.WakeToRun}}, @{n='Every';e={$_.Triggers[0].Repetition.Interval}}

Enable-ScheduledTask  -TaskName "apartment-manager Command Dispatcher"   # Telegram polling ON
Disable-ScheduledTask -TaskName "apartment-manager Command Dispatcher"   # OFF
```

### Change how often it wakes (elevated)
```powershell
$t = Get-ScheduledTask -TaskName "apartment-manager Command Dispatcher"
